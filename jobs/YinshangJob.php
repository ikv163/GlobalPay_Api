...
class YinshangJob extends BaseObject implements \yii\queue\RetryableJobInterface
{
    public $trans_id;

    public function execute($queue)
    {
        \Yii::info($this->trans_id, 'YinshangJob_Begin');
        try {
            $trans = TransactionFlow::find()->where(['trans_id' => $this->trans_id, 'trans_status' => 0])->one();
            if (!$trans) {
                \Yii::info($this->trans_id . '不存在', 'YinshangJob_noExsit');
                return false;
            }
            ...
            \Yii::info(json_encode($orders), 'YinshangJob_Orders');
            ...
            \Yii::info(json_encode($res), 'YinshangJob_Result_'.$order['order_id']);
            if ($res['result'] == 0) {
